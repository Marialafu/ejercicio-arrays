...
const confirmGmailMail = mail => {
  mail[0].includes('gmail') ||
  mail[1].includes('gmail') ||
  mail[2].includes('gmail')
...
const extractLetters = name => {
  ...
  const addNumber = excludeLetters + Math.floor(Math.random() * 99);
...
const generateUserName = allName => {
  ...
  const findSurname = allName[0].substring(
    allName[0].indexOf(' ') + 1,
    allName[0].length
  );

  const excludeLetters =
    findName.charAt(0).toUpperCase() + findName.charAt(1).toUpperCase();
...
const applyDiscount = price => {
  ...
  const discount =
    randomNumber < price
      ? `${price - price * 0.1} (10% de descuento)`
      : `${price - price * 0.2} (20% de descuento)`;
  return discount;
};
...
const discountsPerPrices = pricesList => {
  ...
  console.log(
    `Precio original: ${
      pricesList[0]
    } || Número originado: ${applyDiscount()} || Precio final: ${firstPrice}`
  );
};
...
const discountPerPrice = precios => {
  ...
  const firstDiscount =
    firstPrice < precios[0]
      ? `${precios[0] - precios[0] * 0.1} (10% de descuento)`
      : `${precios[0] - precios[0] * 0.2} (20% de descuento)`;

  console.log(
    `Precio original: ${precios[0]} || Número originado: ${firstPrice} || Precio final: ${firstDiscount}`
  );

  const secondPrice = Math.floor(Math.random() * 100);
  const secondDiscount =
    secondPrice < precios[1]
      ? `${precios[1] - precios[1] * 0.1} (10% de descuento)`
      : `${precios[1] - precios[1] * 0.2} (20% de descuento)`;

  console.log(
    `Precio original: ${precios[0]} || Número originado: ${secondPrice} || Precio final: ${secondDiscount}`
  );
};
...
const calculatePriceAccount = price => {
  ...
  const dividedPriceResult =
    dividedPrice % 2 === 0 ? dividedPrice : Math.round(dividedPrice);
  return dividedPriceResult;
};

const calculateListPrices = listCount => {
  console.log(
    `Cuenta original: ${listCount[0]} | Dividido: ${
      listCount[0] / 3
    } | Precio final: ${calculatePriceAccount(listCount[0])}`
  );

  console.log(
    `Cuenta original: ${listCount[1]} | Dividido: ${
      listCount[1] / 3
    } | Precio final: ${calculatePriceAccount(listCount[1])}`
  );
};
...
const priceAccount = price => {
  ...
  const firstDividedCountResult =
    firstDividedCount % 2 === 0
      ? firstDividedCount
      : Math.round(firstDividedCount);
  ...
  const secondDividedCountResult =
    secondDividedCount % 2 === 0
      ? secondDividedCount
      : Math.round(secondDividedCount);
...
const calculateClientsAges = bornYear => {
  ...
  if (firstClientsAge > 18 || secondClientsAge > 18 || thirdClientsAge > 18) {
    ...
    console.log('Todos son menores de edad *procede a venderles alchohol');
  }
};
...
const clientList = names => {
  ...
  names.push('Lucía');
  console.log(
    `${names[names.length - 1]} se añadió a la lista, ahora hay ${
      names.length
    } personas.`
  );
};
...
const stockAvailable = products => {
  ...
  console.log(
    `El producto ${
      products[aleatoryItem]
    } se ha eliminado de la lista. Tienes ${
      products.length - 1
    } productos en stock.`
  );
};
...
const waitingList = clients => {
  clients.unshift('Fernando');
  console.log(
    `La lista de espera es: ${clients}. Al último, ${
      clients[clients.length - 1]
    }, le tocará esperar 30 minutos`
  );
};
...
const tennisTournamentAttendees = atendees => {
  atendees.pop() + atendees.push('Diego');
